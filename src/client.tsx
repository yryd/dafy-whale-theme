...
// ---------------------------------------------------------------------------
//
// 为什么不用官方的 `SlotMap` 契约：**DSH 并没有把 `ctx.slots` 类型化**——
// 在所有 DSH 类型包里都搜不到 `slots: XxxService` 这样的服务声明，
// 官方自己的客户端插件也全是 JS 直接调 `ctx.slots.inject`。
// 既然拿不到官方的 `ctx.slots` 类型，这里就只声明所需的最小类型面；
// 插槽注册的正确性改由**运行时测试**兜底（见 test/client.test.mjs：
// 注册名字、id、order、成对注册、组件可渲染性、32 个字段全覆盖）。
// 同理，那 8 个只提供 SlotMap 声明合并的类型包已从 devDependencies 移除——
// 装了也参与不了检查，留着只是僵尸依赖。
